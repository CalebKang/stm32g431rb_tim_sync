...
int main(void)
{
  ...
  LL_TIM_SetCounter(TIM8, LL_TIM_GetAutoReload(TIM8));
  ...
  while (1)
  {
    HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, LL_TIM_GetCounter(TIM1));
    HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_2, DAC_ALIGN_12B_R, LL_TIM_GetCounter(TIM8));
    /* USER CODE END WHILE */
...
static void MX_TIM8_Init(void)
{
  ...
  LL_TIM_SetSlaveMode(TIM8, LL_TIM_SLAVEMODE_GATED);
